...
for (var propName in snake) {
	if (propName[0] === 's') {
		snake[propName] = 's';
	}
}

console.log(snake.says);
...
function removeItem(arrStr, str) {
	while (arrStr.indexOf(str) != -1) {
		if (arrStr.indexOf(str) != -1) {
			arrStr.splice(arrStr.indexOf(str), 1);
		}
	}
...
function getValueOfProperty(obj, nameProp) {
...
function callerBack(holla, back){
	holla(back + ' back');
	console.log('Showing function: ', holla);
	console.log('Showing back: ', back);
}